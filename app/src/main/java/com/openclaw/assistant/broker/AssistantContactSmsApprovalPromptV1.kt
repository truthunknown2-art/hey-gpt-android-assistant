...
internal object AssistantContactSmsApprovalPromptV1 {
    private const val CHANNEL_ID = "assistant_contact_sms_approvals"

    fun present(context: Context, proposalId: String): Boolean {
        val appContext = context.applicationContext
        val intent = Intent(appContext, AssistantContactSmsApprovalActivityV1::class.java)
            .setData(Uri.parse("openclaw-assistant://contact-sms/$proposalId"))
            .putExtra(AssistantContactSmsApprovalActivityV1.EXTRA_PROPOSAL_ID, proposalId)
    ...
    private fun postNotification(context: Context, intent: Intent, proposalId: String): Boolean {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            context.getSystemService(NotificationManager::class.java).createNotificationChannel(
                NotificationChannel(
                    CHANNEL_ID,
                    context.getString(R.string.assistant_contact_sms_channel),
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle(context.getString(R.string.assistant_contact_sms_title))
            .setContentText(context.getString(R.string.assistant_contact_sms_notification))
